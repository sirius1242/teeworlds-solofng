	...
	switch(m_ActiveWeapon)
	{
		case WEAPON_HAMMER:
		{
			...
			for (int i = 0; i < Num; ++i)
			{
				...
				bool MeltHit = GameServer()->m_pController->IsTeamplay() && pTarget->GetPlayer()->GetTeam() == GetPlayer()->GetTeam() && pTarget->IsFreeze();
				...
				if (MeltHit)
					pTarget->Freeze(m_pPlayer->GetCID(), WEAPON_MELT);
			}
...
void CCharacter::Freeze(int Killer, int Weapon)
{
	...
	{
		g_pData->m_Weapons.m_Ninja.m_Duration = g_Config.m_SvHammerMelt * 1000;
